// $Id: mozgMLP_2.cc,v 1.1 2006/03/18 21:53:18 lightdruid Exp $
// 
// mozgMLP_2.cc
...
// neuron output functions (see math.hh)
    const OUT_FUNC mozgMLP::output_func [7] = { mozg_weinet,
						mozg_exp,
						mozg_th,
						mozg_lin,
						mozg_rbf,
...
// neuron output functions derivatives (see math.hh)
    const OUT_FUNC_DERIV mozgMLP::output_func_deriv [7] = { mozg_D_weinet,
							    mozg_D_exp,
							    mozg_D_th,
							    mozg_D_lin,
							    mozg_D_rbf,
...
// put net_inputs to the net inputs
  void mozgMLP::
      ...
      if (flag_of_bias_term)
	{
      ...
      else num_inputs = layer[0].outputs_num;
...
// propagate layer's inputs through the layer
  void mozgMLP::
    compute_lay_outputs(const struct_layer& LOWER,
			struct_layer* UPPER)
    {
      if (!UPPER->flag_RBF)
	multiply(*UPPER->weights,
		 *LOWER.outputs,
		 UPPER->nets);
      
      else multiply(*UPPER->weights,
		    *LOWER.outputs,
		    UPPER->RBFinputs,
		    UPPER->nets);
...
// the error_over_outputs
  void mozgMLP::
    compute_output_errors(mozgflt* target,
			  bool outerrflag)
    {
      struct_layer& lay = layer[layers_num - 1];
      
      subtract(*lay.outputs,
	       target,
	       diff);
...
// place square of a error vector module in the error_over_outputs
	...
	case 2:
	  if (lay.flag_RBF
	      ||(nonstand_func_type==1)) {
	    
	    copy(OVER_UNIT,
		 temp_vector);
	    subtract(*temp_vector,
		     *lay.outputs,
		     temp_vector);
...
// remove division by zero:
	...
	case 3:
// remove division by zero:
	  if (error_over_outputs==1) error_over_outputs -= EPS;
...
//      *temp_vector /= *lay.outputs;break; // temp_vector_i=t_i/o_i
	...
	OUT_FUNC_DERIV output_func_deriv =
	  lay.lay_output_func_deriv;
	
	mozgint i = lay.outputs_num;
	
	if (beta == 1.) while (i--)
	  {
	    *errors++ = *lay_diff++ * (*output_func_deriv)(*outputs,
							   *nets,
							   *sigmas);
	    outputs++;
	    nets++;
	  ...
	  {
	    *errors++ = *lay_diff++ * beta
	      * (*output_func_deriv)(*outputs,
				     (beta * *nets),
				     *sigmas);
	    outputs++;
	    nets++;
...
// compute neuron errors for the current layer
  void mozgMLP::
    compute_lay_errors(const struct_layer& prev_layer,
		       const matrix<mozgflt,mozgint>& prevlay_weights,
...
//      vec_t delta = 0.0;
      ...
      OUT_FUNC_DERIV output_func_deriv =
	cur_layer->lay_output_func_deriv;
      
      multiply(*prev_layer.errors,
	       prevlay_weights,
	       cur_layer->errors);
...
// and gets "layer's delta errors" named layer's errors in the mozg
      if (beta == 1.) while (j--)
	{
	  *cur_errors++ *= (*output_func_deriv)(*cur_outputs,
						*cur_nets,
						*cur_sigmas);
	  cur_outputs++;
	  cur_nets++;
	...
	{
	  *cur_errors++ *= beta * (*output_func_deriv)(*cur_outputs,
						       (beta * *cur_nets),
						       *cur_sigmas);
	  cur_outputs++;
	  cur_nets++;
...
// adjust weights for current layer
  void mozgMLP::
    adjust_lay_weights(const struct_layer& prev_layer,
		       struct_layer* cur_layer)
    {
      mozgflt* lay_inputs;
      mozgflt* lay_nets;
...
// acceleration:
	    ...
	    {
	      *cur_sigmas_in_chunk +=
		-eta * *lay_errors * *lay_nets++ / (*cur_sigmas);
...
// (optimesed CPU time)
  void mozgMLP::
    o_adjust_lay_weights(const struct_layer& prev_layer,
			 struct_layer* cur_layer)
    {
      mozgflt* lay_inputs;
      mozgflt* lay_nets;
...
//   replace copying by pointers exchanging
	    ...
	    copy (*cur_layer->errors,
		  cur_layer->sigma_pderivs);
...
//   replace copying by pointers exchanging
	...
	*cur_layer->pderivs += *cur_layer->pderivs_in_chunk;
	...
	if (flag_RBF)
	  {
	    copy (*cur_layer->errors,
		  cur_layer->sigma_pderivs_in_chunk);
...
// replace copying by pointer exchanging
	  ...
	  if (output_flag_RBF)
	    {
	      vector<mozgflt,mozgint>* tmp1   =
		output_layer.sigma_prev_pderivs;
	      output_layer.sigma_prev_pderivs = output_layer.sigma_pderivs;
	      output_layer.sigma_pderivs      = tmp1;
	    }
	  multiply(*layer[layers_num-2].outputs,
		   *output_layer.errors,
		   output_layer.pderivs);

	  if (output_flag_RBF)
	    {
	      copy (*output_layer.errors,
...
// replace copying by pointer exchanging
		...
		zero (output_layer.sigma_pderivs);
	      ...
	      copy (*output_layer.errors,
		    output_layer.sigma_pderivs_in_chunk);

	      *output_layer.sigma_pderivs_in_chunk *=
		*layer[layers_num-1].nets;
...
// remove devision by zero:
      ...
      for (mozgint i = layers_num - 1; i > 1; i--)
	{
	  ...
	  compute_lay_pderivs(*prev_layer,
			      layer[i-2],
			      *prev_weights,
			      cur_layer);
	}
      ...
      for (mozgint i=1; i < layers_num; i++)
	{
	  ...
	  if (chunk>1)
	    if (flag_of_extensions)
	      adjust_chunk_lay_weights(*prev_layer,
				       cur_layer);
	    else
	      o_adjust_chunk_lay_weights(*prev_layer,
					 cur_layer);
	  
	  else {
	    if (flag_of_extensions)
	      adjust_lay_weights(*prev_layer,
				 cur_layer);
	    
	    else
	      o_adjust_lay_weights(*prev_layer,
				   cur_layer);
	  }
	}
      if (sigma_Langevin) sigma_Langevin *= sigma_Langevin_decrease;
    }
...
// make cut of second order term:
		  if (fabs(second_order_term) > CUT_QUICKPROP)
		      second_order_term = signum (second_order_term)
			* CUT_QUICKPROP;
		...
		while (k--)
		  {
			...
			second_order_term = *sigma_pderiv / tmp
			  * *last_sigma_delta;
...
// make cut of second order term:
			...
			*last_sigma_delta = -eta * *sigma_pderiv
			  * (((*sigma_pderiv * *sigma_prev_pderiv) > 0)
			     ? 1.
			     : 0.)
			  + second_order_term
			  + sigma_delta;
...
//    adjust with extensions:
		  ...
		  && fabs(*weight_eta) > min_abs_weight_etas)
		{
		  *weight_eta *= ((*pderiv * *prev_pderiv) > 0.)
		  ...
		  if (*weight < max_abs_weight
		      && *weight > min_abs_weight )
		    *weight += - *weight_eta * signum(*pderiv);
		}
	      weight++;
	      weight_eta++;
