// $Id: vector_matrix_io.cc,v 1.1 2006/03/18 21:53:18 lightdruid Exp $
// 
// vector_matrix_io.cc
...
namespace mozg {
  ...
  template std::ofstream& operator << (std::ofstream& output,
				  const vector<mozgflt,mozgint>& vector);
  template std::ofstream& operator << (std::ofstream& output,
				  const vector<mozgint,mozgint>& vector);
  ...
  template <class vec_t, class vec_size_t>
			   ...
			   vector<vec_t,vec_size_t>& vector)
    {
      ...
      while (size--) input >> (*temp++);
      
      return input;
    }
  template std::ifstream& operator >> (std::ifstream& input,
				  vector<mozgflt,mozgint>& vector);
  template std::ifstream& operator >> (std::ifstream& input,
				  vector<mozgint,mozgint>& vector);
  ...
  template std::ofstream& operator << (std::ofstream& output,
				  const matrix<mozgflt,mozgint>& matr);
  template std::ofstream& operator << (std::ofstream& output,
				  const matrix<mozgint,mozgint>& matr);
  ...
  template std::ifstream& operator >> (std::ifstream& output,
				  matrix<mozgflt,mozgint>& matr);
  template std::ifstream& operator >> (std::ifstream& output,
				  matrix<mozgint,mozgint>& matr);
